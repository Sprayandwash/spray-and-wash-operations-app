...
2. **A PowerShell script that locates the zip itself and runs it, no matter where it was saved or what PowerShell's current folder is.** A plain `.\spray-wash-staging-app.zip` relative path is not enough on its own - the requester is often not sitting in the same folder the browser saved the zip into (their default download location, a project folder, OneDrive, etc). Always give the auto-locating form:

   ```powershell
   $zip = Get-ChildItem -Path . -Filter "spray-wash-staging-app*.zip" -ErrorAction SilentlyContinue | Select-Object -First 1
   if (-not $zip) {
       Write-Host "Searching your whole user folder for the zip - this can take a minute..."
   ...
   if (-not $zip) {
       Write-Host "Could not find spray-wash-staging-app*.zip anywhere under your user folder."
   } else {
       Write-Host "Found: $($zip.FullName)"
       Set-Location $zip.Directory
       Expand-Archive -Path $zip.FullName -DestinationPath .\staging-app -Force
       npx --yes http-server .\staging-app -p 4174 -c-1
   }
   ```

   It checks the current folder first, then - only if needed - searches the whole user profile, so it works whether the zip landed in Downloads, a OneDrive project folder, the Desktop, or anywhere else. Then open `http://127.0.0.1:4174` in a browser. Before testing, confirm `staging-app/STAGING-README.txt` references only the Staging Supabase project and never the production ref - the same safety check `staging-training-review.yml` performs in CI.

This handoff is a standing requirement for every Step 8-9A phase, not a one-off request. The command given must always be this self-locating form - never one that assumes or hardcodes a specific folder (including a plain relative path that assumes the current directory).
